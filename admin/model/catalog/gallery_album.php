...
class ModelCatalogGalleryalbum extends Model {	
	...
	public function addAlbum($data) {
		...
		if (isset($data['album_image'])) {
			foreach ($data['album_image'] as $album_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "gallery_image SET album_id = '" . (int)$album_id . "', image = '" . $this->db->escape(html_entity_decode($album_image['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$album_image['sort_order'] . "'");
			}
		}
	...
	public function editAlbum($album_id, $data) {
	    ...
	    $this->db->query("DELETE FROM " . DB_PREFIX . "gallery_image WHERE album_id = '" . (int)$album_id . "'");
	...
	public function getAlbumImages($album_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "gallery_image WHERE album_id = '" . (int)$album_id . "' ORDER BY sort_order ASC");
